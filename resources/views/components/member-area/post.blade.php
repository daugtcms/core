...
@endphp
<div class="bg-white rounded-xl overflow-hidden h-64 bg-cover bg-center relative shadow-lg px-6 py-2 flex flex-col justify-center" style="background-image: url('{{$backgroundImage}}')">
    ...
    <div class="relative z-10 pointer-events-none">
        @if(isset($post->blocks['template']['attributes']['courseSection']))
            ...
            @endphp
            <h2 class="text-white/70 text-xl font-semibold flex items-center -space-x-0.5"><span>{{$tempCourse->name}}</span> @svg('dot') <span>{{$tempSection->name}}</span></h2>
        @endif
        <h2 class="text-white text-3xl font-semibold">{{$post->title}}</h2>
        <div class="flex items-center mt-4 gap-x-2.5">
            <x-sitebrew::avatar class="h-12 w-12"></x-sitebrew::avatar>
            <div>
                <p class="text-primary-50 text-xl font-medium">{{$post->user->name}}</p>
                <p class="text-primary-100 -mt-1.5 text-lg text-base">{{$post->created_at->diffForHumans()}}</p>
            </div>
        </div>
    </div>
